...
class CommandHistory:
    """
    Manages the history of executed commands and provides undo/redo capabilities.
    
    DESIGN PATTERN: Command + Memento (stores command history)
    PURPOSE: Track and manage command execution history
    SOLID PRINCIPLE: Single Responsibility - only manages command history
    ...
    def undo(self) -> bool:
        ...
        Returns:
            bool: True if undo was successful, False if no commands to undo
        """
        if not self.history:
            return False
    ...
    def redo(self) -> bool:
        ...
        Returns:
            bool: True if redo was successful, False if no commands to redo
        """
